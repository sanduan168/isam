...
vector<Factor*> glc_remove_node(Slam& slam, Node* node, int& clique_size, bool sparse, GLC_Reparam* rp) {
  ...
  vector<Factor*> new_glc_factors;
//  std::cout << "clique_size: " << clique_nodes.size() << std::endl;
  clique_size = clique_nodes.size();
  if(clique_nodes.size() == 0) {
    return new_glc_factors;
  }

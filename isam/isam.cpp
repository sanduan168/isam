...
void incremental_slam() {
  ...
  for (; loader->more_data(&next_step); step = next_step) {
    ...
    GLC_RootShift rs;
    //glc landmark
    if(loader->point_nodes().size() > 0 && step > 10 && step < 6600) {
//      std::cout << "\nlandmark_size " << current_landmark_size << std::endl;
//      std::cout << "size: " << *loader->point_nodes()[current_landmark_size] << std::endl;
//      slam.print_graph();
//      vector<Factor *> felim = glc_elim_factors(loader->point_nodes()[current_landmark_size]); //usefull for local managment of factors
      int clique_size = 0;
      vector<Factor *> fnew = glc_remove_node(slam, loader->point_nodes()[current_landmark_size], clique_size, true, &rs); // root shifted
      if (fnew.size() > 0 || clique_size > 0) {
//        std::cout << "new.size: " << fnew.size() << std::endl;
//        std::cout << "clique.size: " << clique_size << std::endl;
        current_landmark_size++;
//        std::cout << "landmark_size: " << current_landmark_size << std::endl;
      }
    }
    //glc pose
//    if(loader->pose_nodes().size() > 0 && step > 10 && step < 6600) {
////      std::cout << "\nlandmark_size " << current_landmark_size << std::endl;
////      std::cout << "size: " << *loader->point_nodes()[current_landmark_size] << std::endl;
////      slam.print_graph();
////      vector<Factor *> felim = glc_elim_factors(loader->point_nodes()[current_landmark_size]); //usefull for local managment of factors
//      int clique_size = 0;
//      vector<Factor *> fnew = glc_remove_node(slam, loader->pose_nodes()[current_landmark_size], clique_size, false, &rs); // root shifted
//      if (fnew.size() > 0 || clique_size > 0) {
////        std::cout << "new.size: " << fnew.size() << std::endl;
////        std::cout << "clique.size: " << clique_size << std::endl;
//        current_landmark_size++;
...
//
//      GLC_RootShift rs;
//      while (current_landmark_size < loader->point_nodes().size()) {
////      std::cout << "\nlandmark_size " << current_landmark_size << std::endl;
//      std::cout << "size: " << *loader->point_nodes()[current_landmark_size] << std::endl;
////      slam.print_graph();
////      vector<Factor *> felim = glc_elim_factors(loader->point_nodes()[current_landmark_size]); //usefull for local managment of factors
//        int clique_size = 0;
//        vector<Factor *> fnew = glc_remove_node(slam, loader->point_nodes()[current_landmark_size], clique_size, false, &rs); // root shifted
//        if (fnew.size() > 0 || clique_size > 0) {
////        std::cout << "new.size: " << fnew.size() << std::endl;
////        std::cout << "clique.size: " << clique_size << std::endl;
//          current_landmark_size++;
